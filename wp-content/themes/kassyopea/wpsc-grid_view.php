	...
	<div class="product_grid_display group">
		<?php while (wpsc_have_products()) :  wpsc_the_product(); ?>
			<div class="product_grid_item product_view_<?php echo wpsc_the_product_id(); ?>">
				...
				<?php if(wpsc_product_on_special()) : ?><span class="sale"><?php _e('Sale', TEXTDOMAIN); ?></span><?php endif; ?>	
				<?php if(get_option('show_images_only') != 1): ?>
					<div class="grid_product_info">
						...
						<?php endif; ?>
							...
							<?php if(wpsc_product_on_special()) : ?>
										<p class="pricedisplay <?php echo wpsc_the_product_id(); ?>"><?php _e('Old Price', TEXTDOMAIN); ?>:<span class="oldprice"><?php echo wpsc_product_normal_price(); ?></span></p>
									<?php endif; ?>
									<p class="pricedisplay <?php echo wpsc_the_product_id(); ?>"><?php _e('Price', TEXTDOMAIN); ?>:<span class="currentprice"><?php echo wpsc_the_product_price(); ?></span></p>
									<?php if(wpsc_show_pnp()) : ?>
										<p class="pricedisplay"><?php _e('Shipping', TEXTDOMAIN); ?>:<span class="pp_price"><?php echo wpsc_product_postage_and_packaging(); ?></span></p>
									<?php endif; ?>	 
							</div><!--close price_container-->
						<?php if(get_option('display_moredetails') == 1) : ?>
							<a href="<?php echo wpsc_the_product_permalink(); ?>" class="more_details"><?php _e( 'More details', TEXTDOMAIN ); ?></a>
						<?php endif; ?> 
					...
					<div class="grid_more_info">
						<form class="product_form"  enctype="multipart/form-data" action="<?php echo wpsc_this_page_url(); ?>" method="post" name="product_<?php echo wpsc_the_product_id(); ?>" id="product_<?php echo wpsc_the_product_id(); ?>" >
                         ...
                         <fieldset><legend><?php _e('Product Options', TEXTDOMAIN); ?></legend>       
						<div class="wpsc_variation_forms">
							...
							<?php if((get_option('display_addtocart') == 1) && (get_option('addtocart_or_buynow') !='1')) :?> 	   
								<?php if(wpsc_product_has_stock()) : ?>
									<input type="submit" value="<?php _e('Add To Cart', TEXTDOMAIN); ?>" name="Buy" class="wpsc_buy_button" id="product_<?php echo wpsc_the_product_id(); ?>_submit_button"/>
								<?php else : ?>
									<p class="soldout"><?php _e('Sorry, sold out!', TEXTDOMAIN); ?></p>
								<?php endif ; ?>
							<?php endif; ?>
		...
		<?php if(wpsc_product_count() == 0):?>
			<p><?php  _e('There are no products in this group.', TEXTDOMAIN); ?></p>
		<?php endif ; ?>
