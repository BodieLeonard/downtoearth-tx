...
function yiw_get_configuration( $settings )
{
	...
	{         
		$var = get_option( "bl_flashslider_$setting" );
...
';
		...
		$slides = get_slides( 'bl_flashslider_slides' );
		
		foreach( $slides as $id => $slide ) :
			 
			$content = str_replace( "\n", '', $slide['tooltip_content'] );
			if ( $content != '' ) $content = wpautop($content);
      ...
      <Image Source=\"".get_bloginfo('template_url')."/piecemaker/contents/flash-preview.png\" />  
  </Flash>\n";
		    }                        
    ...
    <Image Source=\"".get_bloginfo('template_url')."/piecemaker/contents/video-preview.jpg\" />  
  </Video>\n";
		    } 
	...
	string_( '  <Settings ', implode( ' ', $fields ), '></Settings>' );
